...
check("deadbang", workflowsDeadFilters([("w", 1, "paths", "!gone/**")], ["src/x"]).isEmpty)
check("rootstar", workflowsDeadFilters([("w", 1, "paths", "**/Cargo.lock")], ["Cargo.lock"]).isEmpty)
check("dirslash", workflowsDeadFilters([("w", 1, "paths", ".vscode/")], [".vscode/settings.json"]).isEmpty)
check("midstar", workflowsDeadFilters([("w", 1, "paths", "a/**/b.txt")], ["a/b.txt"]).isEmpty)
check("stilldead", workflowsDeadFilters([("w", 1, "paths", "gone/**")], ["src/a"]).count == 1)
check("quant-alive", workflowsDeadFilters([("w", 1, "paths", "**/*.yaml?")], ["a/b.yaml"]).isEmpty)
check("quant-yam", workflowsDeadFilters([("w", 1, "paths", "**/*.yaml?")], ["a.yam"]).isEmpty)
check("quant-dead", workflowsDeadFilters([("w", 1, "paths", "**/*.yaml?")], ["a.yml"]).count == 1)
check("plus", workflowsDeadFilters([("w", 1, "paths", "a+.txt")], ["aaa.txt"]).isEmpty)
print(bad.isEmpty ? "CORE OK" : "APART: " + bad.joined(separator: ","))
''')
