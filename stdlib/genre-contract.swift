...
// never made cannot be refused — so absence is reported by the importer as a
// guard line, at the address in the contract that declares the field. The same
// division the ownership door already draws: a rule reaching outside its zone
// is judged, a pattern matching no file is named beside it.
